	...
	basic_string<T, Allocator>& basic_string<T, Allocator>::append_sprintf_va_list(const value_type* pFormat, va_list arguments)
	{
		...
		int nReturnValue;
			...
			if constexpr (std::is_same_v<value_type, char>)
			    nReturnValue = eastl::Vsnprintf((char8_t*)nullptr, 0, (const char8_t*)pFormat, arguments);
		    else
			    nReturnValue = eastl::Vsnprintf((value_type*)nullptr, 0, pFormat, arguments);
		...
		if (nReturnValue > 0)
		{
			...
			if constexpr (std::is_same_v<value_type, char>) {
				nReturnValue = eastl::Vsnprintf((char8_t*)internalLayout().BeginPtr() + nInitialSize, (size_t)(nReturnValue + 1), (const char8_t*)pFormat, arguments);
			}
			else {
